...
if [ -z "$JDK" -a -s "$HOME/.IntelliJIdea2018.2/config/idea.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.IntelliJIdea2018.2/config/idea.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.IntelliJIdea2018.2/config/idea$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.IntelliJIdea2018.2/config/idea$BITS.vmoptions"
